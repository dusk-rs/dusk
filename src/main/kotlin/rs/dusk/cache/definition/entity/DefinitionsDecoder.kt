...
import rs.dusk.cache.definition.impl.ItemDefinitionLoader
...
interface DefinitionsDecoder<T, D : DefinitionDecoder<T>> where T : Definition, T : Extra {
	...
	companion object {
		...
		fun toIdentifier(name : String) =
			removeTags(name.toLowerCase().replace(underscoreChars, "_")).replace(chars, "").replace("&#39;", "")
	}
}
...
//	single(createdAtStart = true) { NPCDefinitionLoader(get(), get()).run(getProperty("npcDefinitionsPath")) }
	single(createdAtStart = true) { ItemDefinitionLoader(get()).run(getProperty("itemDefinitionsPath")) }
