...
internal class InterfaceLoaderTest {
	...
	private lateinit var yamlIO : YamlIO<InterfaceData>
	
	@BeforeEach
	fun setup() {
		yamlIO = mockk()
		loader = spyk(InterfaceDetailsLoader())
	}
	...
	fun `Load details`() {
		val raw = mapOf(
			"interface_name" to mapOf(
				...
				"components" to mapOf(
					"component_name" to 0,
					"component" to mapOf("id" to 1, "options" to mapOf("Option1" to 0, "Examine" to 3))
				)
			)
		)
		...
		val expected = mapOf(
			"interface_name" to InterfaceDetail(
				...
				data = data,
				components = mapOf(
					"component_name" to InterfaceComponentDetail(0, "component_name"),
					"component" to InterfaceComponentDetail(
						1,
						"component",
						options = arrayOf("Option1", "", "", "Examine")
					)
				)
			)
		)
	...
	fun `Load multiple details`() {
		val raw = mapOf(
			"interface_name" to mapOf(
				"id" to 1,
				"type" to "interface_type",
				"components" to mapOf("component_name" to 1, "component_name_two" to 2)
			),
			"interface_name_two" to mapOf(
				"id" to 2,
				"type" to "interface_type",
				"components" to mapOf("component_name" to 3)
			)
		)
		val data = InterfaceData()
		val types = mapOf("interface_type" to data)
		...
		val expected = mapOf(
			"interface_name" to InterfaceDetail(
				...
				data = data,
				components = mapOf(
					"component_name" to InterfaceComponentDetail(1, "component_name"),
					"component_name_two" to InterfaceComponentDetail(2, "component_name_two")
				)
			),
			"interface_name_two" to InterfaceDetail(
				id = 2,
				name = "interface_name_two",
				type = "interface_type",
				data = data,
				components = mapOf("component_name" to InterfaceComponentDetail(3, "component_name"))
			)
		)
		assertEquals(expected, results)
	}
	...
	fun `Missing components is empty map`() {
		val raw = mapOf(
			"interface_name" to mapOf(
				"id" to 1,
				"type" to "interface_type",
				"components" to mapOf<String, Int>()
			)
		)
		val data = InterfaceData()
		val types = mapOf("interface_type" to data)
	...
	fun `Load from file`() {
		...
		val raw = mapOf("interface_name" to mapOf("id" to 1, "type" to "interface_type"))
		every { yamlIO.read<Map<String, Map<String, Any>>>(path) } returns raw
	...
	fun `Load all`() {
		...
		every { yamlIO.read<Map<String, Map<String, Any>>>(detailsPath) } returns detailData
		every { yamlIO.read<Map<String, Map<String, Any>>>(typesPath) } returns typesData
		...
		val expected = InterfaceDetails(
			mapOf(
				...
				"toplevel" to InterfaceDetail(
					id = 2,
					name = "toplevel",
					type = "root",
					data = InterfaceData(-1, -1, 0, 0)
				),
				"toplevel_full" to InterfaceDetail(
					id = 3,
					name = "toplevel_full",
					type = "root",
					data = InterfaceData(-1, -1, 0, 0)
